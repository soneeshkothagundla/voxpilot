...
2. You **hold the push-to-talk key** (default: **Right Ctrl**) and say,
...
| `hotkey` | `ptt_key` | `ctrl_r` | Push-to-talk key (pynput name, e.g. `ctrl_r`, `f9`, `cmd_r`). |
...
|---|---|---|
| **Push-to-talk** | **Hold Right Ctrl** (`ctrl_r`) | Hold to record; release to transcribe and act. In `toggle` mode, tap to start, tap to stop. |
